...
pub fn parse_string<I: Interrupt>(input: &str, int: &I) -> Result<Expr, IntErr<String, I>> {
    let tokens = crate::lexer::lex(input, int)?;
    let (res, remaining) = parse_expression(tokens.as_slice()).map_err(IntErr::get_error)?;
